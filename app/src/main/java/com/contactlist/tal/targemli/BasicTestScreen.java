...
public class BasicTestScreen extends AppCompatActivity {
    static int i = 0;
    String[] answers;
    String[] englishWords;
    String name;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (BasicWords.newsGame) {
            basicWords();
        }
    }

    private String[] basicWords() {
        SharedPreferences sharedPreferences = getSharedPreferences("saveContinueGameBasic", 0);
        Resources res = getResources();
        this.englishWords = res.getStringArray(R.array.basicWords);
        this.answers = res.getStringArray(R.array.basicWordsAnswers);
        if (i == 0) {
    ...
    public void check(View view) {
        if (BasicWords.newsGame) {
            SharedPreferences sharedPreferencess = getSharedPreferences("saveContinueGameBasic", 0);
            if (this.translate.getText().toString().trim().equals(this.answers[i])) {
                ...
                sharedPreferencess = getSharedPreferences("saveContinueGameBasic", 0);
