...
public class LiteraryTestScreen extends AppCompatActivity {
    static int i;
    String[] answers;
    String[] englishWords;
    EditText translate;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (LiteraryWords.newsGame) {
            basicWords();
        }
    }

    private String[] basicWords() {
        SharedPreferences sharedPreferences = getSharedPreferences("saveContinueGameLiterary", 0);
        Resources res = getResources();
        this.englishWords = res.getStringArray(R.array.LiteraryWords);
        this.answers = res.getStringArray(R.array.LiteraryWordsAnswers);
        if (i == 0) {
    ...
    public void check(View view) {
        if (LiteraryWords.newsGame) {
            SharedPreferences sharedPreferencess = getSharedPreferences("saveContinueGameLiterary", 0);
            if (this.translate.getText().toString().trim().equals(this.answers[i])) {
                ...
                sharedPreferencess = getSharedPreferences("saveContinueGameLiterary", 0);
