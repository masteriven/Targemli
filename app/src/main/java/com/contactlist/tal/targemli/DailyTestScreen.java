...
public class DailyTestScreen extends AppCompatActivity {
    static int i;
    String[] answers;
    String[] englishWords;
    String name;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (DailyWords.newsGame) {
            basicWords();
        }
    }

    private String[] basicWords() {
        SharedPreferences sharedPreferences = getSharedPreferences("saveContinueGameDaily", 0);
        Resources res = getResources();
        this.englishWords = res.getStringArray(R.array.dailyWords);
        this.answers = res.getStringArray(R.array.dailyWordsAnswers);
        if (i == 0) {
    ...
    public void check(View view) {
        if (DailyWords.newsGame) {
            SharedPreferences sharedPreferencess = getSharedPreferences("saveContinueGameDaily", 0);
            if (this.translate.getText().toString().trim().equals(this.answers[i])) {
                ...
                sharedPreferencess = getSharedPreferences("saveContinueGameDaily", 0);
